...
Chapter 9 of the book introduces a completely updated and comprehensive set of hands-on labs and exercises designed to let architects command each tool in their arsenal. In practice that material grew far beyond what the printed book could hold — well over five hundred pages of step-by-step procedures, screenshots, product showcases, and tooling references. Rather than overwhelm the primary text, this content lives here as a dedicated **supplemental download** and **subsequent domain category labs** so it remains fully available to readers.

The supplement is a single document that introduces the foundational concepts leading into the domain labs. The labs themselves are organized into **categories** that mirror the major defensive-security domains, and each category is available as a separate download. Each category pairs a domain narrative and other material that could not fit within the book — the history of the tool class, the commercial and enterprise landscape, comparison tables, and the rationale behind the open-source choices — with one or more complete, validated, build-it-yourself labs. The labs are designed to be stood up from scratch on a single host (current builds target **Debian 13**) using **free and open-source tooling**, so the entire supplement can be completed at no software cost.
